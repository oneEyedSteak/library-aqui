...
export default function BooksDirector({ bookTrack }) {
  return (
   ...
   lg:border-gray-400 bg-white  lg:rounded-b-1
    lg:rounded-r p-4 flex flex-col justify-between leading-normal"
        >
          ...
          <h4 className="text-gray-600 font-bold text-2xl mb-2">{bookTrack.title}</h4>
          <p className="text-gray-600 mb-5">
            {bookTrack.notereqform}
          </p>
          ...
          <div>
            ...
            <p className=" font-thin ">
              { bookTrack.selectPosition}
            </p>
          </div>
          <div>
            <small className="font-bold mr-2">Requestee Department</small>
            <p className=" font-thin ">
              { bookTrack.selectDepartment}
            </p>
          </div>
          <div>
            <small className="font-bold">Publication Date</small>
            <p className=" font-thin">
              {new Date(bookTrack.pubdate).toDateString()}
            </p>
          </div>
          <div>
            <small className="font-bold">Requested Date</small>
            <p className=" font-thin">
              {new Date(bookTrack.date).toDateString()}
            </p>
            <Link href={`/track-book/${bookTrack.requestID}`}>
              <button
                type="button"
