...
export default function RequestedCardsFinance({ booksFinanceDisplay }) {
  return (
   ...
   lg:border-gray-400 bg-white  lg:rounded-b-1
    lg:rounded-r p-4 flex flex-col justify-between leading-normal"
        >
          ...
          <h4 className="text-gray-600 font-bold text-2xl mb-2">{booksFinanceDisplay.title}</h4>
          <p className="text-gray-600 mb-5">
            {booksFinanceDisplay.notereqform}
          </p>
          <div className="flex justify-between space-x-1 text-gray-600 text-xs">
            <div>
              ...
              <p className="text-gray-600 mb-2 font-thin ">
                {booksFinanceDisplay.authorName}
              </p>
            </div>
          </div>
          <div>
            ...
            <p className="text-gray-600 mb-2 font-thin ">
              {booksFinanceDisplay.price}
            </p>
          </div>
          <div>
            <small className="font-bold mr-2">Requestee ID</small>
            <p className=" font-thin ">
              { booksFinanceDisplay.userID}
            </p>
          </div>
          <div>
            <small className="font-bold mr-2">Name</small>
            <p className=" font-thin ">
              { booksFinanceDisplay.requestee}
            </p>
          </div>
          <div>
            <small className="font-bold mr-2">Position</small>
            <p className=" font-thin ">
              { booksFinanceDisplay.selectPosition}
            </p>
          </div>
          <div>
            <small className="font-bold mr-2">Requestee Department</small>
            <p className=" font-thin ">
              { booksFinanceDisplay.selectDepartment}
            </p>
          </div>
          <div>
            <small className="font-bold">Publication Date</small>
            <p className=" font-thin">
              {new Date(booksFinanceDisplay.pubdate).toDateString()}
            </p>
          </div>
          <div>
            <small className="font-bold">Requested Date</small>
            <p className=" font-thin">
              {new Date(booksFinanceDisplay.date).toDateString()}
            </p>
            <Link href={`/approve-books-finance/${booksFinanceDisplay.requestID}`}>
              <button
                type="button"
