...
import BooksFinance from '../components/BooksFinance';
import validateSession from '../lib/session';

export const getServerSideProps = async (context) => {
  try {
    const result = await
    mysql.query('SELECT * FROM requestform WHERE approvalVpaaPayment = 1 and approvalDirector = 1');
...
booksDisplayFinance && booksDisplayFinance.map((reqbook) => (
  <BooksFinance booksDisplayFinance={reqbook} />
))
