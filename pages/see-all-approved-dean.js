...
export const getServerSideProps = async (context) => {
  try {
    ...
    const result = await mysql.query(`SELECT * FROM requestform WHERE approvalDean = 1 AND
  ...
  const postRequestedBooks = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: 'approvalDateDean', // accessor is the "key" in the data
        Cell: ({ row: { values } }) => (
          <div>
            {new Date(values.approvalDateDean).toDateString()}
          </div>
        ),
      ...
      <section className="max-w-screen bg-base min-h-screen mx-auto ">
        ...
        <form className=" p-14 bg-blue-200 rounded-md my-16 w- mx-auto h-auto w-auto shadow-lg ">
          <div className="flex-shrink-0 flex content-around items-center">
            ...
            <h1 className="text-3xl font-bold text-black-600 "> Approved Books by Dean</h1>
