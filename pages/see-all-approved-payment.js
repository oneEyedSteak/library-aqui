...
export const getServerSideProps = async (context) => {
    try {
        ...
        const result = await mysql.query(`SELECT * FROM requestform WHERE approvalFinancePayment = 1  `);

        const post = JSON.parse(JSON.stringify(result));
        return {
            props: { approvalFinance: post, account },
        };
    } catch (error) {
...
export default function AllApprovedFinance({ approvalFinance }) {
    ...
    const postRequestedBooks = useMemo(
        () => [
            ...
            },
            {
                ...
                accessor: 'approvalFinancePayment', // accessor is the "key" in the data
                Cell: ({ row: { values } }) => (
                    <div>
                        {values.approvalFinancePayment ? 'Yes' : 'No'}
                    </div>
                ),
            },
            {
                Header: 'Approved Date',
                accessor: 'approvalFinanceDate', // accessor is the "key" in the data
                Cell: ({ row: { values } }) => (
                    <div>
                        {new Date(values.approvalFinanceDate).toDateString()}
                    </div>
                ),
            ...
            <section className="max-w-screen bg-base min-h-screen mx-auto ">
                ...
                <form className=" p-14 bg-blue-200 rounded-md my-16 w- mx-auto h-auto w-auto shadow-lg ">
                    <div className="flex-shrink-0 flex content-around items-center">
                        ...
                        <h1 className="text-3xl font-bold text-black-600 ">Approved Price by Finance</h1>
                    ...
                    <div className="text-xs shadow-md w-full mt-10 p">
                        <label htmlFor="selectDepartment" className="block ">
                            ...
                            <ReactTable data={approvalFinance} columns={postRequestedBooks} />
                        </label>
                    </div>
