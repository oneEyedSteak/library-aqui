...
export const getServerSideProps = async (context) => {
  try {
    const result = await
    mysql.query('SELECT * FROM requestform WHERE status = 2');
    const session = await validateSession(context);

    const post = JSON.parse(JSON.stringify(result));
    return {
      props: { booksToPublish: post, session },
    };
  } catch (error) {
    return { props: { post: false } };
  }
};
export default function seeAllBooksPublisher({ booksToPublish }) {
  const columns = useMemo(
    () => [
...

      {
        ...
        accessor: 'action',
        Cell: ({ row: { values } }) => (
          <Link href={`/books-to-publish/${values.requestID}`}>
